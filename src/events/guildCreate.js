...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`Andromeda has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const AndromedaColor = guild.roles.cache.find(r => r.name === '#Seagrass');
    if (AndromedaColor) await guild.me.roles.add(AndromedaColor);
  } catch (err) {
    client.logger.error(err.message);
  }
